...
def print_filter_summary(total_input: int, total_passed: int,
...
├──────────────────────────────────────────────────────────────┤
│  收盘价: {close:.2f}元   封板时间: {seal}   成交额: {amount/1e8:.1f}亿
│
│  五维明细:
...
def print_backtest_summary(results: Dict):
    ...
    print(f"  📊 回测结果摘要")
    print(f"{'='*60}")
    print(f"  回测区间: {results.get('start_date', '—')} ~ {results.get('end_date', '—')}")
    print(f"  交易日数: {results.get('trading_days', 0)}")
    print(f"  总信号数: {results.get('total_signals', 0)}")
    print(f"  胜率:     {results.get('win_rate', 0):.1%}")
    print(f"  平均收益: {results.get('avg_return', 0):.2%}")
    print(f"  最大盈利: {results.get('max_win', 0):.2%}")
    print(f"  最大亏损: {results.get('max_loss', 0):.2%}")
